...
def main():
    ...
    for file_name, file_url in files_to_download.items():
        ...
        completed_process_wget = subprocess.run(
            f"wget -t 2 {file_url} -O {output_file}", shell=True, capture_output=True
        )
        if completed_process_wget.returncode != 0:
            data_download_errors.append(completed_process_wget.stderr.decode("utf-8"))
        ...
        if len(data_download_errors) > 0:
            ...
            with open("/wms/artic-pipeline/artic-ont-pipeline-run-job.yaml", "r") as f:
                job_to_submit = yaml.load(f.read(), Loader=yaml.FullLoader)
                # Assigning unit id to job (id of run)
                job_to_submit["metadata"]["name"] = (
                    f"artic-ont-pipeline-run-job-" f"{file_name.lower()}"
                )
                # Submitting run id as arg to job
                job_to_submit["spec"]["template"]["spec"]["containers"][0]["args"] = [
                    file_name
                ]
