...
def process_item(item):
    ...
    try:
        ...
        for count, link in enumerate(links, start=1):
            # set the output file to force overwrite in case this is not the first download attempt
            wget_args = f"--directory-prefix /data/{run_id}_input --no-verbose --output-document {run_id}_{count}.fastq.gz"
